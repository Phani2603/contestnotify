...
const FeatureTitle: React.FC<FeatureTitleProps> = ({ children }) => {
  return (
...
const FeatureDescription: React.FC<FeatureDescriptionProps> = ({ children }) => {
  return (
    <p
      className={cn(
...
export const SkeletonTwo: React.FC = () => {
  ...
  const getRotation = (idx: number): number => {
    // Use a deterministic pattern for rotation values
    // This creates a sequence of rotation values that appear random but are consistent
    const rotationValues = [
      -1.94, 2.28, -5.56, -0.67, 5.52, // First row
      -3.00, -0.66, -1.32, -2.15, -8.26 // Second row
    ];
    return rotationValues[idx % rotationValues.length];
  };
      ...
      <div className="flex flex-row">
        {images.map((image, idx) => (
          <motion.div
            ...
            style={{
              rotate: getRotation(idx + 5), // Offset for second row
            }}
            variants={imageVariants}
            whileHover="whileHover"
            whileTap="whileTap"
